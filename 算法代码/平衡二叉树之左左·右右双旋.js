 ...
 * @Date: 2022-01-19 10:01:15
 * @LastEditTime: 2022-01-19 10:09:16
...
function Change(root) {
    ...
    if(Math.abs(leftDeep - rightDeep) < 2) {
        ...
        newRoot.right = Change(newRoot.left); // 右旋之后再次判断，因为右旋改变了结构
        newRoot = Change(newRoot); // Change newRoot.left 之后树的结构再次可能发生了改变
        return newRoot;
    } else {
        ...
        newRoot.left = Change(newRoot.left);  // 左旋之后再次判断，因为左旋改变了结构
        newRoot = Change(newRoot); // Change newRoot.right 之后树的结构再次可能发生了改变
        return newRoot;
    }
}
